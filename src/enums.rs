...
#[derive(Debug)]
enum GivenResult<T, E> { // We can have an enum return a generic!
...
fn create_car_colour_blue() -> CarColour { // Use -> to define return type.
...
fn check_under_five(num_check: u8) -> GivenResult<u8, String> { // We return the value, or a string with the rror
    if num_check < 5 { 
